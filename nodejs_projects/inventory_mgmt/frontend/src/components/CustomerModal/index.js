...
function CustomerModal({ customers, actionType }) {
    ...
    return (
        <div className='w-2/3 fixed top-10 z-30 bg-white rounded-sm shadow-md p-5 left-1/2 -translate-x-1/2'>
            ...
            <form className="[&>*]:flex [&>*]:flex-col [&>*]:justify-between [&>*]:p-2 mt-5" onSubmit={handleSubmit}>
                <div>
                    <select className="border px-4 py-2" onChange={(e) => setDetails({ ...details, id: e.target.value })}>
                    <option value=''>...</option>
                    {
                        customers.map((c, i) => {
                            return (
                ...
                <div>
                    ...
                    <input type="text" id="amafaranga" value={details.reduced} autoComplete="amafaranga" className="border px-4 py-1" onChange={(e)=>setDetails({...details, reduced: e.target.value})} />
                </div>
                <button type='submit' className="mt-3 border-0 rounded-sm py-1 px-2 bg-blue-600 text-white">
                    Add Product
                </button>
            </form>
        </div>
