...
function Modal({ products, setProducts, setTempProducts, type }) {
    ...
    const handleAddProduct = () => {
        if (
            ...
            (productDetail.buyingPrice === '') || (productDetail.quantity === '') || (productDetail.transactionType === '')
        ) {
        ...
        setProductDetail({ id: products.length + 1, name: '', customer: '', buyingPrice: '', quantity: '', date: '', transactionType: '' })
        ...
        <div className="[&>*]:flex [&>*]:flex-col [&>*]:justify-between [&>*]:p-2 mt-5">
            <div>
                ...
                <select
                    name="customer"
                    id="customer"
                    value={productDetail.customer}
                    className="border px-4 py-1"
                    onChange={(e) => { setProductDetail({ ...productDetail, customer: e.target.value }) }}
                >
                    <option value="" className="text-slate-300">...</option>
            ...
            <div>
                ...
                <select
                    name="customer"
                    id="customer"
                    value={productDetail.transactionType} className="border px-4 py-1"
                    onChange={(e) => { setProductDetail({ ...productDetail, transactionType: e.target.value }) }}
                >
                    <option value="" className="text-slate-300">...</option>
            ...
            <div>
                ...
                <input
                    type="text"
                    id="price"
                    value={productDetail.buyingPrice}
                    autoComplete="price"
                    className="border px-4 py-1"
                    onChange={(e) => { setProductDetail({ ...productDetail, buyingPrice: e.target.value }) }}
                />
            </div>
            <div>
                ...
                <input
                    type="text"
                    id="total_amount"
                    value={productDetail.buyingPrice * productDetail.quantity}
                    className="border px-4 py-1" disabled
            ...
            <div>
                ...
                <input
                    ...
                    autoComplete="date"
                    className="border px-4 py-1"
                    onChange={(e) => { setProductDetail({ ...productDetail, date: e.target.value }) }}
                />
            </div>
        </div>
    )
